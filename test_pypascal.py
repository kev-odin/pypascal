...
class LexerTestCase(unittest.TestCase):
    ...
    def test_lexer_new_tokens(self):
        from pypascal import ASSIGN, DOT, ID, SEMI, BEGIN, END
        records = (
            (':=', ASSIGN, ':='),
            ('.', DOT, '.'),
            ('number', ID, 'number'),
...
class InterpreterTestCase(unittest.TestCase):
    def makeInterpreter(self, text):
        from pypascal import Lexer, Parser, Interpreter
        lexer = Lexer(text)
        parser = Parser(lexer)
